...
export default class MobileBind extends React.Component<any, any> {
  ...
  componentWillMount() {
    ...
    loadUserProfileInfo().then(res => {
      dispatch(endLoad())
      if(res.code === 200) {
        this.setState(_.merge({ canSubmit: res.msg.canSubmit }, this.state, res.msg))
      } else {
  ...
  handleChangePhone(e) {
    ...
    if(value && this.state.code) {
      this.setState({ mobile: value, disable: false })
    } else {
      this.setState({ mobile: value, disable: true })
    }
  }
  ...
  render() {
    const { sending, seconds, bindMobile, code,canSubmit, oversea } = this.state
    let { mobile, weixinId } = this.state

      ...
      <div className="landing-page-container">
            ...
            <Swiper height='16rem'>
              {pageBanners.map((banner, index) =>
                <img key={index}
                     src={banner.imageUrl}
                     onClick={() => {
                       BigData.sendBigData({ module: '打点', function: '着陆页', action: '点击Banner' });
        ...
        <div className="nav">
          <ul>
            ...
            }}><i className="iconfont icon-play"></i><p>大咖直播</p></li>
            <li onClick={() => {
              this.context.router.setState({ pageScrollY: window.pageYOffset });
              this.context.router.push('/rise/static/home/articleList');
            }}><i className="iconfont icon-light"></i><p>职场干货</p></li>
            <li onClick={() => {
              this.context.router.setState({ pageScrollY: window.pageYOffset });
              this.context.router.push('/rise/static/home/activities');
            }}><i className="iconfont icon-group"></i><p>校友活动</p></li>
          </ul>
        </div>
        {/*-----  加入圈外模块  -----*/}
        {
          ...
          <div className="join-qw panel-qw">
            ...
            <div className="panel-body">
              {programsFlows.map((item, index) => <JoinItem item={item}
                                                            key={index}
                                                            handerOrder={this.asstWechatData.bind(this)}></JoinItem>)}
            </div>
          </div>
        }
        {/*-----  大咖直播模块   -----*/}
        {
          ...
          <div className="super-playing panel-qw">
            <div className="panel-header">大咖直播<span className="more"
                                                    onClick={() => {
                                                      this.context.router.setState({ pageScrollY: window.pageYOffset });
                                                      this.context.router.push('/rise/static/home/lives');
                                                    }
            ...
            <div className="panel-body">
              {livesFlows.map((item, index) => <LivesItem livesItem={item}
                                                          key={index}
                                                          handleMess={this.getActivityInfo.bind(this)}></LivesItem>)}
            </div>
          </div>
        }
        {/*-----  文章精选模块  -----*/}
        {
          ...
          <div className="article panel-qw">
            <div className="panel-header">文章精选<span className="more"
                                                    onClick={() => {
                                                      this.context.router.setState({ pageScrollY: window.pageYOffset });
                                                      this.context.router.push('/rise/static/home/articleList');
                                                    }
            ...
            <div className="panel-body">
              {articlesFlows.map((item, index) => <ArticleItem articles={item}
        ...
        <div className="activity panel-qw">
          <div className="panel-header">校友活动<span className="more"
                                                  onClick={() => {
                                                    this.context.router.setState({ pageScrollY: window.pageYOffset });
                                                    this.context.router.push('/rise/static/home/activities');
                                                  }}>更多</span></div>
          <div className="panel-body">
            {activitiesFlows.map((item, index) => <ActivityItem activity={item}
                                                                key={index}
                                                                finishActivity={this.getActivityInfo.bind(this)}></ActivityItem>)}
          </div>
        </div>
        }
        {showSubscribeAlert && <SubscribeAlert asstWechat={this.state.asstWechat}
                                               show={showSubscribeAlert}
                                               closeFunc={() => this.setState({ showSubscribeAlert: false })}/>}
        <Layout description={layoutDescription}></Layout>
        <div className="bottom-text">我也是有底线的...</div>
        {/*-----   弹框模块  -----*/}

        <Alert show={this.state.applySuccess.isShowPassNotify}

        ...
        <div className="label-container">
          <span className="tips">选择标签:</span>
          {
            this.state.labels.map((item, seq) => {
              return (
                <div className={`label-item ${item.selected?"selected":''}`}
                     style={{marginLeft:`${this.labelMargin}px`,marginRight:`${this.labelMargin}px`}}
                     onClick={()=>this.clickLabel(item.selected,seq)}>{item.name}</div>
              )
            })
          }
        </div>
      ...
      <div className="discuss-page">
        <div className="submit" style={{minHeight:`${this.props.height}px`}}>
          ...
          <Editor ref="editor" moduleId={this.props.moduleId}
                  value={this.state.comment} placeholder="离开页面前请提交，以免内容丢失。"/>

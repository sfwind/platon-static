...
import { loadWarmUpAnalysis, answer, getOpenStatus, consolidationStatus, loadWarmUpDiscuss, discuss, deleteComment } from './async'
...
export default class Warumup extends React.Component<any, any> {
      ...
      if(code === 200) {
        ...
        if(practice) {
          ...
          if(idx !== -1) {
            ...
            this.setState({ list: msg, practiceCount: msg.practice.length, currentIndex,
            ...
            if(practice) {
              ...
              this.setState({ list: msg, practiceCount: msg.practice.length, currentIndex, selected,
  ...
  render() {
    const {
      ...
      integrated, placeholder, openStatus, analysis, submit, data
    } = this.state
    const { openConsolidation = true } = openStatus
        ...
        if(showDiscuss){
          return (
            <Discuss isReply={isReply} placeholder={placeholder} limit={1000}
                     submit={() => this.onDiscuss()} onChange={(v) => this.onChange(v)}
                     cancel={() => this.cancel()}/>
          )
        }else{
          return (

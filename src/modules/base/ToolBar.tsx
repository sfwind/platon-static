...
export class ToolBar extends React.Component<any,any> {
  ...
  componentWillMount() {
    ...
    if(window.location.pathname === '/rise/static/plan/main' ||
        window.location.pathname === '/rise/static/learn' || window.location.pathname === '/rise/static/plan/study'){
      tabIndex = 0;
  ...
  render() {
    ...
    const renderIcon = (item,idx) => {
      ...
      if(item.key === 0) {
        return (
          <img className={`left ${(this.state.tabs.length===5 && idx === 2)?'bigger_img':''}`}  src={tabIndex == item.key?bar.activeIcon:bar.icon}/>
        );
      } else {
        return (

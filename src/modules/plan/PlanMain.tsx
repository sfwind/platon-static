...
export class PlanMain extends React.Component <any, any> {
 ...
 componentWillUnmount() {
  ...
  render() {
    const {
      currentIndex, planData, showScoreModal, showCompleteModal, showConfirmModal,
      selectProblem, riseMember, riseMemberTips, defeatPercent, showWarningModal, chapterList, isHistory,
      showEmptyPage
    } = this.state
    const {location} = this.props
    const {
    ...
    console.log('showEmptyPage', showEmptyPage)
    return (
        ...
        {showEmptyPage ? (
          ...
          <div className="rise-main">
            ...
            <Modal
              header={{
                replace: true,
                children: <AssetImg width={107} height={83} url="http://www.iqycamp.com/images/fragment/finish_modal3.png"/>
              }}
              buttons={[{click: () => this.confirmNextPlan(), content: "下一小课"}, {
                click: () => this.closeCompleteModal(),
                content: "取消"
              }]}
            ...
            <Modal show={showConfirmModal}
                   buttons={[{click: () => this.nextPlan(), content: "确定"}, {
                     click: () => this.closeConfirmModal(),
                     content: "取消"
                   }]}>
              <div className="content">
